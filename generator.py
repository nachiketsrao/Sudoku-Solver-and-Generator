...
import generating_unique_new_generator
from tkinter import *
...
#hints_taken = 0
#######################################################################
def GenerateSudokuToSolve(level, CurrentUserObject): 
#######################################################################    
    global root,l,copy_l
...
##########################################################################    
    l,copy_l=generating_unique_new_generator.generating_sudoku(level)  # copy_l solved sudoku
    print("Main l  :",l)
######################################################################3### 
    def click(event):
...
###################################################################################
    ...
    def check_all_inputs(count,final):
        if count>0:
            ...
            global copy_l,init_time
            preparing_for_exit()
            button_clear_all['state']="disabled"
            button_pause['state']='disabled'
            button_hint['state']='disabled'
            button_guess['state']='disabled'
            if final==copy_l:
                ...
                print("Previous score= ",original_control_room.points)
                original_control_room.num_score.config(text=original_control_room.points+current_score)
                original_control_room.points+=current_score
                #original_control_room.user_rank=original_control_room.leaderboard()
                ...
                for i in lis:
                    if len(i)>0:
                        if i[0] == CurrentUserObject.name :
                            ...
                            break
                print("This is the list after updation : ",lis)       
                ...
                #temp_rank,total=original_control_room.leaderboard()
                #original_control_room.rank.config(text="Your rank is : "+str(temp_rank))
                #---------------------------------------------
            else:
...
###############################################################################
    ...
    def save_button(): 
        ...
        print(init_time)
        CurrentUserObject.edit_csv(get_data(info_l)[0] , copy_l, init_time, hints_taken, level) # get data gives all values in sudoku
         # level will be 1,2 or 3
        
        print("Sudoku successfully saved oola la la le o")  # saving the sudoku here
...
###############################################################################
    ...
    time_control(init_time)
    #original_control_room.wait_window.destroy()
    root.mainloop()
    
#########################################################################################################    
#GenerateSudokuToSolve(1)